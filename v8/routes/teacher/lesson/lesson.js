const express                       = require("express"),
      ...
      Category                      = require("../../../models/teacher/lessonCategories"),
...
router.get("/teacher/classroom/:id/lesson/:lesson_id/edit", (req, res)=>{
    Lesson.findById(req.params.lesson_id, (err, foundLesson)=>{
        if (err) {
            ...
            res.render("teacher/lesson/edit", {category_id: req.params.id, lesson: foundLesson });
        }
    });
});
...
router.get("/lesson/:lesson_id/show", (req, res)=>{
    Lesson.findById(req.params.lesson_id).populate("content").exec((err, lessonShow)=>{
        if(err){
            ...
            res.render("teacher/lesson/show", {lesson: lessonShow});
        ...
        if(err){
            ...
            Content.findById(req.params.content_id, (err, content)=>{
                if(err){
                    ...
                    res.render("teacher/lesson/content/contentEdit", {lesson_id: req.params.lesson_id, content: content})
                }
            });
        }
    });
